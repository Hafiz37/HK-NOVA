#!/usr/bin/env bash
# HK-NOVA Uninstaller
# Usage: curl -fsSL https://raw.githubusercontent.com/your-organization/hk-nova/main/uninstall.sh | sudo bash
# Version: 1.5
set -euo pipefail
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║       HK-NOVA Uninstaller              ║${NC}"
...
if [ ! -d "$INSTALL_DIR" ]; then
    fail "HK-NOVA is not installed at $INSTALL_DIR"
fi

# ── Confirmation ───────────────────────────────────────────────
warn "This will remove HK-NOVA from your system."
echo ""
echo -e "  Install directory: ${YELLOW}${INSTALL_DIR}${NC}"
echo -e "  Service user:      ${YELLOW}${SERVICE_USER}${NC}"
echo -e "  Database:          ${YELLOW}${INSTALL_DIR}/hk-nova.db${NC}"
...
To reinstall:
  curl -fsSL https://raw.githubusercontent.com/your-organization/hk-nova/main/install.sh | sudo bash
EOF
    ok "Data preserved. Note left at ${INSTALL_DIR}/README-UNINSTALLED.txt"
    
    # Remove everything except data
    find "$INSTALL_DIR" -mindepth 1 -maxdepth 1 ! -name 'hk-nova.db' ! -name 'backups' ! -name '.env' ! -name 'README-UNINSTALLED.txt' -exec rm -rf {} + 2>/dev/null || true
else
    # Remove everything
...
echo -e "  To reinstall:      ${YELLOW}curl -fsSL https://raw.githubusercontent.com/your-organization/hk-nova/main/install.sh | sudo bash${NC}"
echo ""
