...
def add_monitoring_job(
    ...
    **kwargs
    ...
    """Add an interval-based monitoring job."""
    ...
    logger.info("Added monitoring job %s with interval %d minutes", job_id, interval_minutes)
    return job_id


def remove_monitoring_job(job_id: str) -> None:
    """Remove a monitoring job."""
    try:
        scheduler.remove_job(job_id)
        logger.info("Removed monitoring job %s", job_id)
    except JobLookupError:
        logger.debug("Monitoring job %s not found (already removed)", job_id)
