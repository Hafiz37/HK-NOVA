...
@router.get("/api/groups/{group_name}/destinations")
async def get_group_destinations(
    group_name: str,
    db: Session = Depends(get_db)
):
    """
    Return default destination_ids untuk group tertentu.
    Used by device form to auto-populate destination checkboxes.
    """
    group = db.query(Group).filter(Group.name == group_name).first()
    
    return {
        "group_name": group_name,
        "default_destination_ids": group.destination_ids if group and group.destination_ids else [],
        "has_defaults": bool(group and group.destination_ids),
        "count": len(group.destination_ids) if group and group.destination_ids else 0,
