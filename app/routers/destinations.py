...
def _build_config(dest_type: str, form: dict) -> dict:
    ...
    if dest_type == "local":
        file_ext = form.get("local_file_extension", "").strip()
        if not file_ext:
            raise ValueError("File extension is required for Local destination")
        
        cfg = {
            "path": form.get("local_path", "./backups").strip() or "./backups",
    ...
    if dest_type == "smb":
        file_ext = form.get("smb_file_extension", "").strip()
        if not file_ext:
            raise ValueError("File extension is required for SMB destination")
    ...
    if dest_type == "telegram":
        file_ext = form.get("telegram_file_extension", "").strip()
        if not file_ext:
            raise ValueError("File extension is required for Telegram destination")
        ...
        if form.get("telegram_compress") == "1":
            cfg["compress"] = True
        return cfg

    # Git-based (git, github, gitea, forgejo)
    file_ext = form.get("git_file_extension", "").strip()
    if not file_ext:
        raise ValueError("File extension is required for Git destination")
...
async def add_destination(
    ...
    if dest_type == "telegram" and config.get("bot_token"):
        config["bot_token"] = dest.encrypt_token(config["bot_token"])
...
async def edit_destination(
    ...
    if dest_type == "telegram" and config.get("bot_token"):
        old_token = (dest.config_json or {}).get("bot_token", "")
        if config["bot_token"] != old_token:
            config["bot_token"] = dest.encrypt_token(config["bot_token"])
...
async def test_destination(dest_id: int, db: Session = Depends(get_db)):
    ...
    if dest.dest_type.value == "smb":
    ...
    elif dest.dest_type.value == "telegram":
        ...
        if config.get("bot_token"):
            try:
                config["bot_token"] = dest.decrypt_token(config["bot_token"])
            except Exception:
                pass
...
async def test_telegram_config(request: Request):
    """Test Telegram bot token and chat ID without saving to database."""
    form = await request.form()
    ...
    if not bot_token or not chat_id:
        return JSONResponse({
            "ok": False,
            "steps": [
                {"step": "validation", "ok": False, "msg": "Bot token and Chat ID are required"}
            ]
        })
    
    from app.modules.destinations.telegram import TelegramDestination
    backend = TelegramDestination()
    result = await backend.test({"bot_token": bot_token, "chat_id": chat_id})
    return JSONResponse(result)
