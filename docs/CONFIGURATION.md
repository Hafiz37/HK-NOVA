...
|----------|---------|-------------|
| `DATABASE_URL` | `sqlite:///./hk-nova.db` | Database connection string |
...
HK-NOVA uses [Apprise](https://github.com/caronc/apprise) for notifications. Configure channels at **Notifications** in the web UI.
...
1. Generate an SSH key:
   ```bash
   ssh-keygen -t rsa -b 4096 -f ~/.ssh/hk-nova
   ```
2. Copy the public key to your devices:
   ```bash
   ssh-copy-id -i ~/.ssh/hk-nova.pub user@device-ip
   ```
3. Add a credential in HK-NOVA with the SSH Key Path set to the private key path
4. For systemd installs, store the key where the service user can read it:
   ```bash
   sudo mkdir -p /opt/hk-nova/.ssh
   sudo cp ~/.ssh/hk-nova /opt/hk-nova/.ssh/
   sudo chown hk-nova:hk-nova /opt/hk-nova/.ssh/hk-nova
   sudo chmod 600 /opt/hk-nova/.ssh/hk-nova
   ```
...
- Dedicated `hk-nova` user (no login shell)
...
```bash
sudo chown -R hk-nova:hk-nova /opt/hk-nova/backups
```
...
**Database locked:**
```bash
sudo lsof /opt/hk-nova/hk-nova.db
```
