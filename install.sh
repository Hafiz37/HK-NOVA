#!/usr/bin/env bash
# HK-NOVA Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/your-organization/hk-nova/main/install.sh | sudo bash
# Version: 1.5
set -euo pipefail
...
# ── Configuration ──────────────────────────────────────────────
REPO_URL="${VIBENET_REPO:-https://github.com/your-organization/hk-nova.git}"
INSTALL_DIR="${VIBENET_DIR:-/opt/hk-nova}"
SERVICE_USER="${VIBENET_USER:-hk-nova}"
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║       HK-NOVA Installer                ║${NC}"
...
SAFE_DIR=$(mktemp -d /tmp/hk-nova-safe.XXXXXX)

if [ -f "$INSTALL_DIR/.env" ]; then
    cp "$INSTALL_DIR/.env" "$SAFE_DIR/.env"
    warn "Existing .env saved — SECRET_KEY and credentials will be preserved"
fi
if [ -f "$INSTALL_DIR/hk-nova.db" ]; then
    cp "$INSTALL_DIR/hk-nova.db" "$SAFE_DIR/hk-nova.db"
...
if [ -f "$SAFE_DIR/hk-nova.db" ]; then
    # Keep a permanent pre-upgrade backup alongside the live DB
    BACKUP_TS=$(date +%Y%m%d_%H%M%S)
    cp "$SAFE_DIR/hk-nova.db" "hk-nova.db.bak_${BACKUP_TS}"
    chown "$SERVICE_USER:$SERVICE_USER" "hk-nova.db.bak_${BACKUP_TS}"
    chmod 600 "hk-nova.db.bak_${BACKUP_TS}"
    ok "Pre-upgrade database snapshot saved as hk-nova.db.bak_${BACKUP_TS}"
    cp "$SAFE_DIR/hk-nova.db" hk-nova.db
    chown "$SERVICE_USER:$SERVICE_USER" hk-nova.db
    chmod 600 hk-nova.db
...
cat > /etc/systemd/system/hk-nova.service <<EOF
[Unit]
Description=HK-NOVA - Network Device Configuration Backup Manager
...
systemctl enable hk-nova >/dev/null 2>&1
systemctl restart hk-nova
ok "Service installed and started"

# ── Wait and verify ────────────────────────────────────────────
info "Waiting for startup..."
sleep 3
if systemctl is-active --quiet hk-nova; then
    ok "HK-NOVA is running"
else
    warn "Service may still be starting — check: journalctl -u hk-nova -f"
fi
...
echo -e "  Logs:      journalctl -u hk-nova -f"
echo -e "  Service:   systemctl {start|stop|restart|status} hk-nova"
echo ""
...
echo ""
echo -e "  To update: curl -fsSL https://raw.githubusercontent.com/your-organization/hk-nova/main/install.sh | sudo bash"
echo ""
