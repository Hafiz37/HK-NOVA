...
if [ "$SECRET_KEY" = "change-me-to-a-random-secret-key" ] || [ "$SECRET_KEY" = "change-me" ] || [ -z "$SECRET_KEY" ]; then
    ...
    if [ -f "$PERSISTED_KEY" ]; then
        ...
        echo "[HK-NOVA] Auto-generated SECRET_KEY (saved to $PERSISTED_KEY)"
    fi
else
    ...
    if [ ! -f "$PERSISTED_KEY" ]; then
        echo "$SECRET_KEY" > "$PERSISTED_KEY"
        chmod 600 "$PERSISTED_KEY"
        echo "[HK-NOVA] Persisted custom SECRET_KEY to $PERSISTED_KEY"
    elif [ "$(cat "$PERSISTED_KEY")" != "$SECRET_KEY" ]; then
        # Env key differs from persisted — use persisted to protect existing credentials
        echo "[HK-NOVA] WARNING: SECRET_KEY in environment differs from persisted key."
        echo "[HK-NOVA] Using persisted key to protect existing encrypted credentials."
        echo "[HK-NOVA] To force a new key, delete $PERSISTED_KEY and re-enter all credentials."
        export SECRET_KEY=$(cat "$PERSISTED_KEY")
    fi
fi
...
echo "[HK-NOVA] Checking database schema..."
python3 - <<'PYEOF'
import os, sqlite3

db_url = os.environ.get("DATABASE_URL", "sqlite:///./data/hk-nova.db")
...
for col, sql in migrations:
    if col not in existing:
        print(f"[HK-NOVA] Adding column: devices.{col}")
        cur.execute(sql)

conn.commit()
conn.close()
print("[HK-NOVA] Schema up to date.")
PYEOF
