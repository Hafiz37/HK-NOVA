...
show_help() {
    echo "HK-NOVA Management Script"
    echo ""
...
" "$ENV_FILE" "$NEW_PASS"
    ...
    if systemctl is-active --quiet hk-nova 2>/dev/null; then
...
show_status() {
    echo ""
    echo -e "${GREEN}HK-NOVA Status${NC}"
    echo ""
    ...
    if systemctl is-enabled --quiet hk-nova 2>/dev/null; then
